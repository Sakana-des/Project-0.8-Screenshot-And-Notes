...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                             ...
                             QLineEdit, QMessageBox, QColorDialog, QScrollArea)
from PyQt5.QtGui import QPixmap, QPainter, QPen, QColor, QFont
from PyQt5.QtCore import Qt, QPoint, QBuffer, QIODevice, QTimer, pyqtSignal, QRect

def upload_to_joplin_resource(token, image_bytes):
    url = f"http://127.0.0.1:41184/resources?token={token}"
    files = {'data': ('screenshot.png', image_bytes, 'image/png')}
    # Joplin requires the props to be a JSON string
    props = json.dumps({"title": "screenshot.png"})
    data = {'props': props}
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Error uploading resource to Joplin: {response.text}")
    except Exception as e:
        print(f"Error uploading resource to Joplin: {e}")
    return None

def create_joplin_note(token, notebook_id, title, body):
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screenshot & Note to Joplin")
        ...
        self.joplin_layout = QHBoxLayout()
        self.inputs_layout.addLayout(self.joplin_layout)
        
        self.joplin_layout.addWidget(QLabel("Joplin Web Clipper Token:"))
        self.input_token = QLineEdit()
        self.input_token.setEchoMode(QLineEdit.Password)
        self.input_token.setPlaceholderText("Paste token here...")
        self.input_token.setStyleSheet("background-color: white; padding: 5px;")
        self.joplin_layout.addWidget(self.input_token)
        
        self.joplin_layout.addWidget(QLabel("Notebook ID (Optional):"))
        self.input_notebook = QLineEdit()
        self.input_notebook.setPlaceholderText("Biarkan kosong untuk default")
        self.input_notebook.setStyleSheet("background-color: white; padding: 5px;")
        self.joplin_layout.addWidget(self.input_notebook)
        
        # Try load from file
        try:
            with open("joplin_config.txt", "r") as f:
                lines = f.read().splitlines()
                if len(lines) >= 1: self.input_token.setText(lines[0])
                if len(lines) >= 2: self.input_notebook.setText(lines[1])
        except:
            pass
            
        self.btn_save = QPushButton("🚀 2. Save to Joplin")
        self.btn_save.setMinimumHeight(50)
        self.btn_save.setStyleSheet("background-color: #2196F3; color: white; font-weight: bold; font-size: 16px; border-radius: 5px;")
        self.btn_save.clicked.connect(self.save_to_joplin)
    ...
    def save_to_joplin(self):
        if not self.original_pixmap:
            QMessageBox.warning(self, "Error", "Silakan ambil screenshot terlebih dahulu!")
            return
            
        token = self.input_token.text().strip()
        notebook_id = self.input_notebook.text().strip()
        desc = self.text_desc.toPlainText()
        
        if not token:
            QMessageBox.warning(self, "Error", "Silakan masukkan Joplin Web Clipper Token.")
            return
            
        # Save config
        with open("joplin_config.txt", "w") as f:
            f.write(f"{token}\n{notebook_id}")
            
        self.btn_save.setEnabled(False)
        self.btn_save.setText("⏳ Mengirim ke Joplin...")
        ...
        # 1. Upload resource
        resource_id = upload_to_joplin_resource(token, image_bytes)
        
        if not resource_id:
            QMessageBox.critical(self, "Error", "Gagal mengunggah gambar ke Joplin. Pastikan aplikasi Joplin berjalan dan Web Clipper diaktifkan.")
            self.btn_save.setEnabled(True)
            self.btn_save.setText("🚀 2. Save to Joplin")
            return
            
        # 2. Create Note Content
        title = f"Screenshot Note - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        body = desc + f"\n\n![screenshot](:/{resource_id})"
        
        # 3. Save Note
        success, response = create_joplin_note(token, notebook_id, title, body)
        
        self.btn_save.setEnabled(True)
        self.btn_save.setText("🚀 2. Save to Joplin")
        
        if success:
            QMessageBox.information(self, "Berhasil!", "Screenshot dan catatan berhasil ditambahkan ke Joplin!")
            self.text_desc.clear()
        else:
            QMessageBox.critical(self, "Error", f"Gagal menambahkan ke Joplin:\n{response}")
